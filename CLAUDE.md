...
- Para las **model-invoked** (`diagnosing-bugs`, `domain-modeling`, `codebase-design`, `tdd`, `research`, `grilling`, `ponytail`, `git-guardrails-claude-code`), reach for them yo solo cuando la situación calza — no hace falta que el usuario las pida ni las nombre.
- Para las **user-invoked** (`to-spec`, `to-tickets`, `triage`, `wayfinder`, `implement`, `handoff`, `ask-matt`, `improve-codebase-architecture`, `setup-matt-pocock-skills`, `code-review-standards-spec`, `graphify`) no puedo dispararlas solo (crean issues, tocan config, commitean) — pero sí puedo y debo **proponerlas proactivamente** cuando algo tiene esa forma ("esto es del tamaño de un `/wayfinder`, ¿lo armo así?") en vez de esperar a que el usuario piense en el nombre correcto. `graphify` se movió acá el 2026-07-14/15 (antes model-invoked, dos sesiones distintas del equipo llegaron al mismo diagnóstico): en Windows una corrida completa desde cero cuesta ~1.2M tokens (medido en este repo), y en macOS ni siquiera corre (Step 1 depende de PowerShell) — cualquiera de los dos motivos alcanza para no dejarlo automático.
...
| Preguntas sobre arquitectura o "¿qué toca esto?" cruzando `apps/`+`services/` | `/graphify` | Grafo de conocimiento del codebase (nodos, comunidades, BFS/DFS). ⚠️ **Sus pasos están en PowerShell y necesitan el paquete Python `graphify` (vía `uv`/`pipx`) — en macOS falla en el Step 1.** No auto-invocable: en Windows una corrida completa mide ~1.2M tokens, y su trigger original era demasiado ancho ("any question about a codebase") para un cuerpo de ~17.5k tokens |
...
| Hay que destrabar un diseño antes de implementar | `/grilling` | Interroga rama por rama hasta entendimiento compartido. Genérica — para este proyecto conviene el comando `/grill-me`, que tiene el stack de Evalutia cargado |
...
Estas 19 son un subconjunto curado del total disponible — el resto de las skills personales del usuario (`premortem`, `grill-with-docs`, etc.) siguen disponibles globalmente sin necesidad de copiarlas acá; se invocan igual con `/nombre-skill` aunque no vivan en este repo.

**`/grill-me` y `/pulir-ventana` no son skills** — son *slash commands* en `.claude/commands/`. Solo se inyectan cuando los tipeás, así que no ocupan contexto de fondo. `/grill-me` se solapa con la skill `grilling`: hacen casi lo mismo, pero el comando tiene todo el stack de Evalutia hardcodeado y la skill es genérica. Si el usuario dice "grilleame esto" sin barra, salta `grilling` (el genérico) — al detectar esa intención, preferir el comando `/grill-me`.
...
- **`/graphify` → `/improve-codebase-architecture` o `/domain-modeling`**: cuando la pregunta es "qué toca esto" en el monorepo (backend/frontend/etl/python-worker cruzados) y ya se invocó `/graphify` explícitamente, el grafo da el mapa de relaciones antes de decidir dónde escanear o qué término formalizar. En macOS esta cadena está **inactiva** — ver la nota de plataforma en la tabla de arriba; mientras tanto, `/improve-codebase-architecture` arranca solo sin el grafo.
