...
### #185 -- `_cerrar_zombis` ahora también cierra filas `etl` sin `subtipo`, mitad de código implementada (2026-08-21)
...
**El issue tiene dos partes; solo se implementó la de código.** VM de producción sin SSH disponible en esta sesión (mismo patrón intermitente ya documentado varias veces, ver #174/#166) -- la parte 1 (cerrar manualmente las 3 filas puntuales en producción, con nota explicativa) sigue **pendiente**, requiere acceso real a la DB de producción.
...
**Queda como acceptance criteria pendiente**: cerrar las 3 filas reales en producción (parte 1) y confirmar contra la DB real que el fix (ya desplegado una vez que la VM vuelva) las hubiera cerrado igual -- y de paso confirmar que ninguna fila real de CALC quedó atrapada por la primera versión del fix antes de esta corrección (no debería, nunca se desplegó a producción).

---

### #170 -- `catalogo_gap`: rastro contable de SKUs huérfanos descartados por el merge de ventas, mitad de código implementada (2026-08-21)
...
**El issue tiene dos partes; solo se implementó la de código.** VM de producción sin SSH disponible en esta sesión (mismo patrón que #185/#174/#166) -- la parte 1 (correr `run_extract_articulos.py` a mano y diagnosticar por qué esos 6 SKUs puntuales nunca sincronizaron) sigue **pendiente**, requiere acceso real al WS/producción.
...
**Queda como acceptance criteria pendiente**: diagnosticar la causa raíz real de `I02855`-`I02860` contra el payload del WS (parte 1), decidir si corresponde agregarlos al catálogo o excluirlos legítimamente, y confirmar contra producción que `catalogo_gap` efectivamente hubiera capturado ese caso puntual una vez que la VM vuelva.
