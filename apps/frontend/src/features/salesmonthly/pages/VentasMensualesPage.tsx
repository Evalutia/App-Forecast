    ...
    <div className="pg-page">
      ...
      <section className="pg-hero">
        ...
        <div className="pg-hero-content">
          <h1 className="pg-title">Ventas Mensuales</h1>
          <p className="pg-subtitle">Agregado mensual de unidades vendidas por SKU.</p>
                ...
                {isLoading ? (
                  Array.from({ length: 10 }).map((_, i) => (
                    <tr key={i}>
                      ...
                      {Array.from({ length: 4 }).map((__, j) => <td key={j}><span className="pg-skeleton pg-skeleton-md" /></td>)}
                    </tr>
                  ))
                ) : items.length === 0 ? (
                  <tr><td colSpan={5} className="pg-empty">No hay resultados.</td></tr>
                ) : (
                  items.map((r, i) => (
                    <tr key={`${String(r['sku'])}-${String(r['year'])}-${String(r['month'])}-${i}`}>
                      <td className="pg-td-key">{String(r['sku'] ?? '')}</td>
                      <td>{String(r['year'] ?? '')}</td>
                      <td>{String(r['month'] ?? '')}</td>
                      <td>{String(r['ventasCantidad'] ?? r['ventas_cantidad'] ?? '')}</td>
                      <td>{String(r['diasConStock'] ?? r['dias_con_stock'] ?? '')}</td>
                    </tr>
                  ))
                )}
              </tbody>
